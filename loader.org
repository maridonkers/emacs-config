...
#+END_SRC
*** Multiple cursors
The package [[https://github.com/magnars/multiple-cursors.el][multiple-cursors]] appears to be installed by default in Emacs (25x).
#+BEGIN_SRC emacs-lisp
...
(global-set-key (kbd "<S-s-up>") 'backward-paragraph)
(global-set-key (kbd "<S-s-down>") 'forward-paragraph)
