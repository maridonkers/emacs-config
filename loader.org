...
#+END_SRC
*** Swiper at point
With swiper, the following [[https://www.emacswiki.org/emacs/SearchAtPoint][key bindings]] can be defined to insert the current symbol/word at point to the swiper minibuffer:
...
Usage: navigate point to relevant location and start search (C-S, C-R). Then use the key mappings defined above to respectively search for symbol or word at point.
*** Mark
#+BEGIN_SRC emacs-lisp
