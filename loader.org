...
#+END_SRC
*** TODO evil mode
VI key bindings for emacs. The TODO is to check for incompabilities with the existing configuration (e.g. keybindings, etc.)
#+BEGIN_SRC emacs-lisp
(use-package evil
  :ensure t
...
it'll give me carpal tunnel syndrome; sticking to OpenBox).
