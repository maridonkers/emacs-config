...
** Revert buffer
#+BEGIN_SRC emacs-lisp
(global-set-key (kbd "C-M-S-f") 'revert-buffer)
#+END_SRC
** Large files
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
(require 'url) ;; Ensure the 'url' library is loaded

(defun my-elixir-hexdocs-search ()
  "Open Elixir HexDocs search with thing at point or selection, properly URL-encoded."
  (interactive)
  (let* ((search-keyword
          (if (use-region-p)
              (buffer-substring-no-properties (region-beginning) (region-end))
            (thing-at-point 'word)))
         (encoded-search-keyword (url-encode-url search-keyword)) ;; URL-encode the keyword
         (search-url (format "https://hexdocs.pm/elixir/search.html?q=%s" encoded-search-keyword)))
    (browse-url search-url)))

;; (global-set-key (kbd "C-c o") 'my-elixir-hexdocs-search)
...
(use-package elixir-mode
    ...
    (:map elixir-mode-map
          ...
          ;; https://hexdocs.pm/elixir/1.17.3/search.html?q=defp
          ("C-c o" . 'my-elixir-hexdocs-search)
          ("<C-M-return>" . lsp-mode)
